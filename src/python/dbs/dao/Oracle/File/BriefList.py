...
class BriefList(object):
    """
    File List DAO class.
    """
    def __init__(self, api):
        """
        Add api, logger and sql.
        """
        self.api = api
	...
	wheresql = "WHERE F.IS_FILE_VALID <> -1"
        ...
        if block_name:
	    joinsql += " JOIN BLOCKS B ON B.BLOCK_ID = F.BLOCK_ID" 
            wheresql += " AND B.BLOCK_NAME = :block_name"
            binds.update({"block_name":block_name})

        if dataset: 
	    joinsql += " JOIN DATASETS D ON  D.DATASET_ID = F.DATASET_ID " 
	...
	if release_version or pset_hash or app_name or output_module_label:
	    joinsql += """
		LEFT OUTER JOIN FILE_OUTPUT_MOD_CONFIGS FOMC ON FOMC.FILE_ID = F.FILE_ID
		LEFT OUTER JOIN OUTPUT_MODULE_CONFIGS OMC ON OMC.OUTPUT_MOD_CONFIG_ID = FOMC.OUTPUT_MOD_CONFIG_ID 
	    """ 
	    
	if release_version:
	    joinsql += " LEFT OUTER JOIN RELEASE_VERSIONS RV ON RV.RELEASE_VERSION_ID = OMC.RELEASE_VERSION_ID"
        ...
        if pset_hash:
	    joinsql += " LEFT OUTER JOIN PARAMETER_SET_HASHES PSH ON PSH.PARAMETER_SET_HASH_ID = OMC.PARAMETER_SET_HASH_ID" 
	    op = ("=", "like")["%" in pset_hash]
	    wheresql += " AND PSH.PSET_HASH %s :pset_hash" % op
	    binds.update(pset_hash = pset_hash)

        if app_name:
	    joinsql += " LEFT OUTER JOIN APPLICATION_EXECUTABLES AEX ON AEX.APP_EXEC_ID = OMC.APP_EXEC_ID" 
	    op = ("=", "like")["%" in app_name]
	...
	if (origin_site_name):
        ...
        if run_num != -1 :
            ...
            joinsql += " JOIN FILE_LUMIS FL on  FL.FILE_ID=F.FILE_ID "
            #
            run_list = []
            wheresql_run_list = ''
            wheresql_run_range = ''
