  	...
  	class Handler (xml.sax.handler.ContentHandler):
		...
		def __init__(self):
			...
			if name == 'path':
				...
				self.sqls['paths'].append("\nINSERT INTO DATASETS ( DATASET_NAME, IS_DATASET_VALID, PRIMARY_DS_ID, PROCESSED_DS_ID, DATA_TIER_ID, DATASET_TYPE_ID, ACQUISITION_ERA_ID, PROCESSING_ERA_ID, PHYSICS_GROUP_ID, XTCROSSSECTION, GLOBAL_TAG, CREATIONDATE, CREATEBY, LASTMODIFICATIONDATE, LASTMODIFIEDBY) VALUES ('%s', '%s', (SELECT PRIMARY_DS_ID FROM PRIMARY_DATASETS WHERE PRIMARY_DS_NAME = '%s'), (SELECT PROCESSED_DS_ID FROM PROCESSED_DATASETS WHERE PROCESSED_DS_NAME = '%s'), (SELECT DATA_TIER_ID FROM DATA_TIERS WHERE DATA_TIER_NAME = '%s'), '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s');" % ( attrs.get('dataset_path'), '1', self.primary_dataset, self.processed_dataset, self.data_tier, '1', '1', '1', '1', '123', 'gtag', self.creation_date, self.created_by, self.creation_date, 'anzar') )

			if name == 'block':
				self.block_name=attrs.get('name')
				#self.sqls['block']="\nINSERT INTO BLOCKS( BLOCK_NAME, DATASET_ID, OPEN_FOR_WRITING, ORIGIN_SITE, BLOCK_SIZE, FILE_COUNT, CREATION_DATE, CREATE_BY, LAST_MODIFICATION_DATE, LAST_MODIFIED_BY) VALUES ('%s', (SELECT DATASET_ID FROM DATASETS WHERE DATASET_NAME = '%s'), '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s');" % ( attrs.get('name'), attrs.get('path'), '1', '1', attrs.get('size'), attrs.get('number_of_files'), attrs.get('creation_date'), attrs.get('created_by'), attrs.get('last_modification_date'), attrs.get('last_modified_by'))
				self.sqls['block']="\nINSERT INTO BLOCKS( BLOCK_NAME, DATASET_ID, OPEN_FOR_WRITING, BLOCK_SIZE, FILE_COUNT, CREATION_DATE, CREATE_BY, LAST_MODIFICATION_DATE, LAST_MODIFIED_BY) VALUES ('%s', (SELECT DATASET_ID FROM DATASETS WHERE DATASET_NAME = '%s'), '%s', '%s', '%s', '%s', '%s', '%s', '%s');" % ( attrs.get('name'), attrs.get('path'), '1', attrs.get('size'), attrs.get('number_of_files'), attrs.get('creation_date'), attrs.get('created_by'), attrs.get('last_modification_date'), attrs.get('last_modified_by'))
			...
			if name == 'file':
				...
				self.sqls['file'].append( "\nINSERT INTO FILES( LOGICAL_FILE_NAME, IS_FILE_VALID, DATASET_ID, BLOCK_ID, FILE_TYPE_ID, CHECK_SUM, EVENT_COUNT, FILE_SIZE, ADLER32, MD5, AUTO_CROSS_SECTION, CREATION_DATE, CREATE_BY, LASTMODIFICATIONDATE, LAST_MODIFIED_BY) VALUES ('%s', '%s', (SELECT DATASET_ID FROM DATASETS WHerE DATASET_NAME='%s'), (SELECT BLOCK_ID FROM BLOCKS WHERE BLOCK_NAME='%s'), '%s','%s', '%s', '%s', '%s','%s', '%s', '%s', '%s', '%s', '%s');" %( attrs.get('lfn'), '1', self.path, self.block_name, '1', attrs.get('checksum'), attrs.get('number_of_events'), attrs.get('size'), attrs.get('adler32'), attrs.get('md5'), 1, attrs.get('creation_date'), attrs.get('created_by'), attrs.get('last_modification_date'), attrs.get('last_modified_by') ) ) 
				#self.sqls['file'].append( "\nINSERT INTO FILES( LOGICAL_FILE_NAME, IS_FILE_VALID, DATASET_ID, BLOCK_ID, FILE_TYPE_ID, CHECK_SUM, EVENT_COUNT, FILE_SIZE, BRANCH_HASH_ID, ADLER32, MD5, AUTO_CROSS_SECTION, CREATION_DATE, CREATE_BY, LASTMODIFICATIONDATE, LAST_MODIFIED_BY) VALUES ('%s', '%s', (SELECT DATASET_ID FROM DATASETS WHerE DATASET_NAME='%s'), (SELECT BLOCK_ID FROM BLOCKS WHERE BLOCK_NAME='%s'), '%s','%s', '%s', '%s', '%s', '%s','%s', '%s', '%s', '%s', '%s', '%s');" %( attrs.get('lfn'), '1', self.path, self.block_name, '1', attrs.get('checksum'), attrs.get('number_of_events'), attrs.get('size'), '1', attrs.get('adler32'), attrs.get('md5'), attrs.get('auto_cross_section'), attrs.get('creation_date'), attrs.get('created_by'), attrs.get('last_modification_date'), attrs.get('last_modified_by') ) ) 
			...
			if name == 'processed_dataset_algorithm':
				self.sqls['processed_dataset_algorithm'].append("\nINSERT INTO DATASET_OUTPUT_MOD_CONFIGS(DATASET_ID, DS_OUTPUT_MOD_CONFIG_ID) VALUES ((SELECT DATASET_ID FROM DATASETS WHERE DATASET_NAME='%s'), (SELECT OUTPUT_MOD_CONFIG_ID FROM OUTPUT_MODULE_CONFIGS WHERE APP_EXEC_ID=(SELECT APP_EXEC_ID FROM APPLICATION_EXECUTABLES WHERE APP_NAME='%s') AND RELEASE_VERSION_ID=(SELECT RELEASE_VERSION_ID FROM RELEASE_VERSIONS WHERE VERSION='%s') AND PARAMETER_SET_HASH_ID=(SELECT PARAMETER_SET_HASH_ID FROM PARAMETER_SET_HASHES WHERE NAME='NO_PSET_HASH')) );" % (self.path, attrs.get('app_executable_name'), attrs.get('app_version')) )
			...
			if name =='run':
				if attrs.get('run_number') not in already_run:
					self.sqls['run'].append("\nINSERT INTO DATASET_RUNS(DATASET_ID, RUN_NUMBER, COMPLETE, EVENT_COUNT, LUMI_SECTION_COUNT, CREATION_DATE, CREATE_BY) VALUES ((SELECT DATASET_ID FROM DATASETS WHERE DATASET_NAME = '%s'), '%s','%s', '%s', '%s', '%s', '%s');" % ( self.path, attrs.get('run_number'), '1', attrs.get('number_of_events'), attrs.get('number_of_lumi_sections'), self.creation_date , self.created_by ) )
					already_run.append(attrs.get('run_number'))
