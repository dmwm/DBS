...
CREATE TABLE RELEASE_VERSIONS (
    ...
    VERSION VARCHAR2(100) CONSTRAINT NN_RV_VERSION NOT NULL,
    CONSTRAINT PK_RV PRIMARY KEY (RELEASE_VERSION_ID),
    CONSTRAINT TUC_RV_VERSION UNIQUE (VERSION)
);
...
CREATE TABLE BRANCH_HASHES (
    ...
    HASH VARCHAR2(700) CONSTRAINT NN_BH_HASH NOT NULL,
    CONTENT CLOB,
...
GRANT SELECT ON FILE_TYPES TO CMS_DBS3_READ_ROLE;
GRANT INSERT, UPDATE ON FILE_TYPES  TO CMS_DBS3_WRITE_ROLE;
GRANT DELETE ON FILE_TYPES  TO CMS_DBS3_ADMIN_ROLE;
...
/* ---------------------------------------------------------------------- */
/* Add table "PATH_TYPES"                                                 */
/* ---------------------------------------------------------------------- */

CREATE TABLE PATH_TYPES (
    PATH_TYPE_ID INTEGER CONSTRAINT NN_PT_PATH_TYPE_ID NOT NULL,
    PATH_TYPE VARCHAR2(100) CONSTRAINT NN_PT_PATH_TYPE NOT NULL,
    CONSTRAINT PK_PT PRIMARY KEY (PATH_TYPE_ID),
    CONSTRAINT TUC_PT_PATH_TYPE UNIQUE (PATH_TYPE)
);
GRANT SELECT ON PATH_TYPES TO CMS_DBS3_READ_ROLE;
GRANT INSERT, UPDATE ON PATH_TYPES TO CMS_DBS3_WRITE_ROLE;
GRANT DELETE ON PATH_TYPES TO CMS_DBS3_ADMIN_ROLE;
...
CREATE TABLE PARAMETER_SET_HASHES (
    ...
    HASH VARCHAR2(64) CONSTRAINT NN_PSH_HASH NOT NULL,
    NAME VARCHAR2(45),
    CONSTRAINT PK_PSH PRIMARY KEY (PARAMETER_SET_HASH_ID),
    CONSTRAINT TUC_PSH_HASH UNIQUE (HASH)
);
...
GRANT SELECT ON SITES TO CMS_DBS3_READ_ROLE;
GRANT INSERT, UPDATE ON SITES TO CMS_DBS3_WRITE_ROLE;
GRANT DELETE ON SITES TO CMS_DBS3_ADMIN_ROLE;

/* ---------------------------------------------------------------------- */
/* Add table "PROCESS_CONFIGURATIONS"                                     */
/* ---------------------------------------------------------------------- */

CREATE TABLE PROCESS_CONFIGURATIONS (
    PROCESS_CONFIG_ID INTEGER CONSTRAINT NN_PC_PROCESS_CONFIG_ID NOT NULL,
    APP_EXEC_ID INTEGER CONSTRAINT NN_PC_APP_EXEC_ID NOT NULL,
    RELEASE_VERSION_ID INTEGER CONSTRAINT NN_PC_RELEASE_VERSION_ID NOT NULL,
    PARAMETER_SET_HASH_ID INTEGER CONSTRAINT NN_PC_PARAMETER_SET_HASH_ID NOT NULL,
    OUTPUT_MODULE_LABEL VARCHAR2(45) DEFAULT 'NONE' CONSTRAINT NN_PC_OUTPUT_MODULE_LABEL NOT NULL,
    CREATION_DATE INTEGER,
    CREATE_BY VARCHAR2(100),
    CONSTRAINT PK_PC PRIMARY KEY (PROCESS_CONFIG_ID),
    CONSTRAINT TUC_PC_1 UNIQUE (APP_EXEC_ID, RELEASE_VERSION_ID, PARAMETER_SET_HASH_ID, OUTPUT_MODULE_LABEL)
);
GRANT SELECT ON PROCESS_CONFIGURATIONS TO CMS_DBS3_READ_ROLE;
GRANT INSERT, UPDATE ON PROCESS_CONFIGURATIONS TO CMS_DBS3_WRITE_ROLE;
GRANT DELETE ON PROCESS_CONFIGURATIONS TO CMS_DBS3_ADMIN_ROLE;

CREATE INDEX ID_PC_1 ON PROCESS_CONFIGURATIONS (RELEASE_VERSION_ID);

CREATE INDEX ID_PC_2 ON PROCESS_CONFIGURATIONS (PARAMETER_SET_HASH_ID);

CREATE INDEX ID_PC_3 ON PROCESS_CONFIGURATIONS (OUTPUT_MODULE_LABEL);

CREATE INDEX ID_PC_4 ON PROCESS_CONFIGURATIONS (APP_EXEC_ID);
...
CREATE TABLE PATHS (
    PATH_ID INTEGER CONSTRAINT NN_PH_PATH_ID NOT NULL,
    PATH_NAME VARCHAR2(500) CONSTRAINT NN_PH_PATH_NAME NOT NULL,
    IS_PATH_VALID NUMBER(1) DEFAULT 1 CONSTRAINT NN_PH_IS_PATH_VALID NOT NULL,
    PRIMARY_DS_ID INTEGER CONSTRAINT NN_PH_PRIMARY_DS_ID NOT NULL,
    PROCESSED_DS_ID INTEGER CONSTRAINT NN_PH_PROCESSED_DS_ID NOT NULL,
    DATA_TIER_ID INTEGER CONSTRAINT NN_PH_DATA_TIER_ID NOT NULL,
    PATH_TYPE_ID INTEGER CONSTRAINT NN_PH_PATH_TYPE_ID NOT NULL,
    ...
    CONSTRAINT PK_PH PRIMARY KEY (PATH_ID),
    CONSTRAINT TUC_PH_PATH_NAME UNIQUE (PATH_NAME)
);
GRANT SELECT ON PATHS TO CMS_DBS3_READ_ROLE;
GRANT INSERT, UPDATE ON PATHS TO CMS_DBS3_WRITE_ROLE;
GRANT DELETE ON PATHS TO CMS_DBS3_ADMIN_ROLE;

CREATE INDEX IDX_PH_1 ON PATHS (PRIMARY_DS_ID);

CREATE INDEX IDX_PH_2 ON PATHS (DATA_TIER_ID);

CREATE INDEX IDX_PH_3 ON PATHS (PROCESSED_DS_ID);

CREATE INDEX IDX_PH_4 ON PATHS (PATH_TYPE_ID);

CREATE INDEX IDX_PH_5 ON PATHS (PHYSICS_GROUP_ID);

CREATE INDEX IDX_PH_6 ON PATHS (ACQUISITION_ERA_ID);

CREATE INDEX IDX_PH_7 ON PATHS (PROCESSING_ERA_ID);

CREATE INDEX ID_PH_8 ON PATHS (CREATIONDATE);

CREATE INDEX ID_PH_9 ON PATHS (CREATEBY);

ALTER TABLE PATHS ADD CONSTRAINT CC_PH_IS_PATH_VALID 
...
CREATE TABLE BLOCKS (
    ...
    PATH_ID INTEGER CONSTRAINT NN_BK_PATH_ID NOT NULL,
...
CREATE INDEX IDX_BK_1 ON BLOCKS (PATH_ID);
...
CREATE TABLE FILES (
    ...
    PATH_ID INTEGER CONSTRAINT NN_FL_PATH_ID NOT NULL,
...
CREATE INDEX IDX_FL_1 ON FILES (PATH_ID);
...
/* ---------------------------------------------------------------------- */
/* Add table "PATH_PROCESS_CONFIGS"                                       */
/* ---------------------------------------------------------------------- */

CREATE TABLE PATH_PROCESS_CONFIGS (
    PATH_PROCESS_CONF_ID INTEGER CONSTRAINT NN_PPC_PATH_PROCESS_CONF_ID NOT NULL,
    PATH_ID INTEGER CONSTRAINT NN_PPC_PATH_ID NOT NULL,
    PROCESS_CONFIG_ID INTEGER CONSTRAINT NN_PPC_PROCESS_CONFIG_ID NOT NULL,
    CONSTRAINT PK_PPC PRIMARY KEY (PATH_PROCESS_CONF_ID)
);
GRANT SELECT ON PATH_PROCESS_CONFIGS TO CMS_DBS3_READ_ROLE;
GRANT INSERT, UPDATE ON PATH_PROCESS_CONFIGS TO CMS_DBS3_WRITE_ROLE;
GRANT DELETE ON PATH_PROCESS_CONFIGS TO CMS_DBS3_ADMIN_ROLE;

CREATE INDEX IDX_PPC_1 ON PATH_PROCESS_CONFIGS (PATH_ID);

CREATE INDEX IDX_PPC_2 ON PATH_PROCESS_CONFIGS (PROCESS_CONFIG_ID);

/* ---------------------------------------------------------------------- */
/* Add table "PATH_PARENTS"                                               */
/* ---------------------------------------------------------------------- */

CREATE TABLE PATH_PARENTS (
    PATH_PARENT_ID INTEGER CONSTRAINT NN_PP_PATH_PARENT_ID NOT NULL,
    THIS_PATH_ID INTEGER CONSTRAINT NN_PP_THIS_PATH_ID NOT NULL,
    PARENT_PATH_ID INTEGER CONSTRAINT NN_PP_PARENT_PATH_ID NOT NULL,
    CONSTRAINT PK_PP PRIMARY KEY (PATH_PARENT_ID)
);
GRANT SELECT ON PATH_PARENTS TO CMS_DBS3_READ_ROLE;
GRANT INSERT, UPDATE ON PATH_PARENTS TO CMS_DBS3_WRITE_ROLE;
GRANT DELETE ON PATH_PARENTS TO CMS_DBS3_ADMIN_ROLE;

CREATE INDEX IDX_PP_1 ON PATH_PARENTS (THIS_PATH_ID);

CREATE INDEX IDX_PP_2 ON PATH_PARENTS (PARENT_PATH_ID);
...
CREATE TABLE PATH_RUNS (
    PATH_RUN_ID INTEGER CONSTRAINT NN_PR_PATH_RUN_ID NOT NULL,
    PATH_ID INTEGER CONSTRAINT NN_PR_PATH_ID NOT NULL,
    RUN_NUMBER INTEGER,
    ...
    CREATION_DATE INTEGER,
    CREATE_BY VARCHAR2(100),
    CONSTRAINT PK_PR PRIMARY KEY (PATH_RUN_ID)
);
GRANT SELECT ON PATH_RUNS TO CMS_DBS3_READ_ROLE;
GRANT INSERT, UPDATE ON PATH_RUNS TO CMS_DBS3_WRITE_ROLE;
GRANT DELETE ON PATH_RUNS TO CMS_DBS3_ADMIN_ROLE;

CREATE INDEX IDX_PR_1 ON PATH_RUNS (PATH_ID);

CREATE INDEX IDX_PR_2 ON PATH_RUNS (RUN_NUMBER);

CREATE INDEX IDX_PR_3 ON PATH_RUNS (EVENT_COUNT);

CREATE INDEX IDX_PR_4 ON PATH_RUNS (LUMI_SECTION_COUNT);

/* ---------------------------------------------------------------------- */
/* Add table "FILE_PROCESS_CONFIGS"                                       */
/* ---------------------------------------------------------------------- */

CREATE TABLE FILE_PROCESS_CONFIGS (
    FILE_PROCESS_CONFIG_ID INTEGER CONSTRAINT NN_FPC_FILE_PROCESS_CONFIG_ID NOT NULL,
    FILE_ID INTEGER CONSTRAINT NN_FPC_FILE_ID NOT NULL,
    PROCESS_CONFIG_ID INTEGER CONSTRAINT NN_FPC_PROCESS_CONFIG_ID NOT NULL,
    CONSTRAINT PK_FPC PRIMARY KEY (FILE_PROCESS_CONFIG_ID)
);
GRANT SELECT ON FILE_PROCESS_CONFIGS TO CMS_DBS3_READ_ROLE;
GRANT INSERT, UPDATE ON FILE_PROCESS_CONFIGS  TO CMS_DBS3_WRITE_ROLE;
GRANT DELETE ON FILE_PROCESS_CONFIGS  TO CMS_DBS3_ADMIN_ROLE;

CREATE INDEX IDX_FPC_1 ON FILE_PROCESS_CONFIGS (FILE_ID);

CREATE INDEX IDX_FPC_2 ON FILE_PROCESS_CONFIGS (PROCESS_CONFIG_ID);
...
CREATE TABLE PROCESSING_ERAS (
    ...
    PROCESSING_ERA_NAME VARCHAR2(100),
    CONSTRAINT PK_PE PRIMARY KEY (PROCESSING_ERA_ID),
    CONSTRAINT TUC_PE_PROCESSING_ERA_NAME UNIQUE (PROCESSING_ERA_NAME)
);
...
GRANT SELECT ON STORAGE_ELEMENTS TO CMS_DBS3_READ_ROLE;
GRANT INSERT, UPDATE ON STORAGE_ELEMENTS  TO CMS_DBS3_WRITE_ROLE;
GRANT DELETE ON STORAGE_ELEMENTS  TO CMS_DBS3_ADMIN_ROLE;
...
GRANT SELECT ON BLOCK_STORAGE_ELEMENTS TO CMS_DBS3_READ_ROLE;
GRANT INSERT, UPDATE ON BLOCK_STORAGE_ELEMENTS TO CMS_DBS3_WRITE_ROLE;
GRANT DELETE ON BLOCK_STORAGE_ELEMENTS  TO CMS_DBS3_ADMIN_ROLE;
...
ALTER TABLE FILES ADD CONSTRAINT PH_FL 
    FOREIGN KEY (PATH_ID) REFERENCES PATHS (PATH_ID) ON DELETE CASCADE;
...
ALTER TABLE PATH_PROCESS_CONFIGS ADD CONSTRAINT PH_PPC 
    FOREIGN KEY (PATH_ID) REFERENCES PATHS (PATH_ID) ON DELETE CASCADE;

ALTER TABLE PATH_PROCESS_CONFIGS ADD CONSTRAINT PC_PPC 
    FOREIGN KEY (PROCESS_CONFIG_ID) REFERENCES PROCESS_CONFIGURATIONS (PROCESS_CONFIG_ID) ON DELETE CASCADE;

ALTER TABLE PATH_PARENTS ADD CONSTRAINT PH_PP 
    FOREIGN KEY (THIS_PATH_ID) REFERENCES PATHS (PATH_ID) ON DELETE CASCADE;

ALTER TABLE PATH_PARENTS ADD CONSTRAINT PH_PP2 
    FOREIGN KEY (PARENT_PATH_ID) REFERENCES PATHS (PATH_ID) ON DELETE CASCADE;

ALTER TABLE PATH_RUNS ADD CONSTRAINT PH_PR 
    FOREIGN KEY (PATH_ID) REFERENCES PATHS (PATH_ID) ON DELETE CASCADE;

ALTER TABLE FILE_PROCESS_CONFIGS ADD CONSTRAINT FL_FPC 
    FOREIGN KEY (FILE_ID) REFERENCES FILES (FILE_ID) ON DELETE CASCADE;

ALTER TABLE FILE_PROCESS_CONFIGS ADD CONSTRAINT PC_FPC 
    FOREIGN KEY (PROCESS_CONFIG_ID) REFERENCES PROCESS_CONFIGURATIONS (PROCESS_CONFIG_ID) ON DELETE CASCADE;
...
GRANT SELECT ON SEQ_FPC TO CMS_DBS3_READ_ROLE;
GRANT SELECT ON SEQ_FT TO CMS_DBS3_READ_ROLE;
GRANT SELECT ON SEQ_PC TO CMS_DBS3_READ_ROLE;
...
GRANT SELECT ON SEQ_PH TO CMS_DBS3_READ_ROLE;
GRANT SELECT ON SEQ_PP TO CMS_DBS3_READ_ROLE;
GRANT SELECT ON SEQ_PPC TO CMS_DBS3_READ_ROLE;
GRANT SELECT ON SEQ_PR TO CMS_DBS3_READ_ROLE;
GRANT SELECT ON SEQ_PSDS TO CMS_DBS3_READ_ROLE;
GRANT SELECT ON SEQ_PSH TO CMS_DBS3_READ_ROLE;
GRANT SELECT ON SEQ_PT TO CMS_DBS3_READ_ROLE;
